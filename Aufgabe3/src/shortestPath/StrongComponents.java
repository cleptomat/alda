...
public class StrongComponents<V> {
	...
	public StrongComponents(DirectedGraph<V> g) {
		var graph = g.invert();
		DepthFirstOrder<V> n = new DepthFirstOrder<>(g);
		List<V> inverted = new LinkedList<V>(n.postOrder());
		Collections.reverse(inverted);
		var visited = new LinkedList<V>();

		var compCount = 0;

		for(var val: inverted) {
			if(!visited.contains(val)) {
				var currentComp = new TreeSet<V>();
				postOrderR(val, visited, currentComp, graph);
				comp.put(compCount++, currentComp);
			}
		}
	}

	private void postOrderR(V v, List<V> l, TreeSet<V> postOrder, DirectedGraph<V> g) {
		l.add(v);
		for (var vertex : g.getSuccessorVertexSet(v))
			if (!l.contains(vertex))
				postOrderR(vertex, l, postOrder, g);
	...
	public String toString() {
		...
		for(var edges: comp.entrySet()) {
			builder.
					append("Component ")
					.append(edges.getKey())
					.append(" : ");
			for (var edge : edges.getValue())
				builder
						.append(edge)
						.append(",");
			builder.append("\n");
