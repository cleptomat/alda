...
public class DepthFirstOrder<V> {
    ...
    public DepthFirstOrder(DirectedGraph<V> g) {
        myGraph = g;
        for (var vertex : myGraph.getVertexSet())
            if (!preOrder.contains(vertex)) {
                numberOfDFTrees++;
                preOrderR(vertex);
                postOrderR(vertex, new LinkedList<>());
            }

    }

    private void preOrderR(V v) {
        preOrder.add(v);
        for (var vertex : myGraph.getSuccessorVertexSet(v))
            if (!preOrder.contains(vertex))
                preOrderR(vertex);
    }

    private void postOrderR(V v, List<V> l) {
        l.add(v);
        for (var vertex : myGraph.getSuccessorVertexSet(v))
