...
public class ScotlandYard {
	...
	public static DirectedGraph<Integer> getGraph() throws FileNotFoundException {
		...
		Scanner in = new Scanner(new File("C:\\Users\\clepto\\Desktop\\HS\\Alda\\Aufgabe3\\src\\shortestPath\\ScotlandYard_Kanten.txt"));
		while (in.hasNextLine()){
			String[] s = in.nextLine().split(" ");
			var v = Integer.parseInt(s[0]);
			...
			if(sy_graph.containsVertex(v) && sy_graph.containsEdge(v,w) && sy_graph.getWeight(v,w) < weight) {
				continue;
			}
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		for (int b : sp) {
			if (a != -1)
				switch ((int) sySp.directed.getWeight(a,b)){
					case 2:
...
class ScotlandYardHeuristic implements Heuristic<Integer> {
	...
	public ScotlandYardHeuristic() throws FileNotFoundException {
		coord = new HashMap<>();
		var in = new Scanner(new File("C:\\Users\\clepto\\Desktop\\HS\\Alda\\Aufgabe3\\src\\shortestPath\\ScotlandYard_Knoten.txt"));
		while (in.hasNextLine()) {
			var split = in.nextLine().split("\\s+");
			var id = Integer.parseInt(split[0]);
			var x = Integer.parseInt(split[1]);
			var y = Integer.parseInt(split[2]);
			coord.put(id, new Point(x, y));
		}
	}
	...
	public double estimatedCost(Integer u, Integer v) {
		var a = coord.get(u);
		var b = coord.get(v);
		return (1/30.0) * Math.sqrt(Math.pow(a.x - b.x, 2) + Math.pow(a.y - b.y, 2));
	}
}
